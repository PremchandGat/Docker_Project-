...
services:
        data_base_os:
                ...
                restart: always
                environment:
                        MYSQL_ROOT_PASSWORD: prem
                        MYSQL_USER: Premchand
                        MYSQL_PASSWORD: prem
...
def httpd_configure_always():
    ...
    os.system('docker container run  -dit --name  prem707054   {0}'.format(image))
    check_httpd_installed_or_not=os.system('docker container exec prem707054 rpm -q httpd')
    os.system('docker container rm -f prem707054')
...
os.system( 'echo /usr/sbin/httpd  >> /root/.bashrc')
        ...
        os.system('wait $!')
        os.system('docker cp prem123.py {}:/root/prem123.py '.format(name))
        os.system('wait $!')
        os.system('docker exec {0} /usr/sbin/httpd'.format(name))
        os.system('wait $!')
        os.system('docker exec {0} python3 /root/prem123.py'.format(name))
        if choice == 'docker':
            ip_container(name)
        else:
...
def delete_image():
        ...
        if con == 'y':
           os.system('docker rmi {0} -f'.format(image))
           os.system('docker images ')
        else :
...
def upload():
        ...
        x=1
        if '/' not in select :
          print('you need to rename your image like :  username/webserver:v1')
          upload = input('New name : ')
          os.system('docker tag {0} {1} '.format(select,upload))
          upload_hub=upload
          x=23
        if x==1:
            upload_hub=select
        os.system('docker login ')
        os.system('docker push {0} '.format(upload_hub))
start = 0
while True:
    if start == 0 :
       titlebar()   
